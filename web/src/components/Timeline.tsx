    ...
    <div className="w-full space-y-8">
      ...
      {avatars.length > 0 && (
        <div className="space-y-3">
            <h3 className="text-sm font-medium text-neutral-400 flex items-center gap-2">
                <ImageIcon className="w-4 h-4" /> avatars antigos
            </h3>
      ...
      <div className="relative pl-4 space-y-6">
        ...
        {events.length > 0 ? events.map((event, index) => (
            ...
            <div className="flex flex-col gap-1">
                ...
                <div className="glass p-4 rounded-xl border border-neutral-800/50 hover:border-neutral-700 transition-colors bg-neutral-900/30">
                    ...
                    <div className="text-sm text-neutral-200">
                      {event.from ? (
                        <span className="text-neutral-500 line-through">{event.from}</span>
                      ) : (
                        <span className="text-neutral-500">vazio</span>
                      )}{" "}
                      <span className="text-neutral-500">→</span>{" "}
                      {event.to ? (
                        <span className="text-white font-medium">{event.to}</span>
                      ) : (
                        <span className="text-neutral-500">vazio</span>
                      )}
                    </div>
          ...
          <div className="text-sm text-neutral-500 pl-2">sem eventos no historico</div>
        )}
      </div>
    </div>
  );
